...
# def load_test_data():
#     meme_df = pd.read_csv(data_path_test)
#     return meme_df
...
def read_identity_terms(identity_terms_path = identity_terms_path):
...
def read_identity_tags(identity_tag_path = identity_tag_path):
...
def clear_identity_list(identity_list, df):
    ...
    for tag in identity_list:
        if len(df.loc[df[tag]==1,'hard_label'].value_counts())<2:
            to_remove.append(tag)
    for tag in to_remove:
        identity_list.remove(tag)
    return identity_list

def read_clear_identity_terms(identity_terms_path = identity_terms_path):
    identity_terms = read_identity_terms(identity_terms_path)
...
def shuffle_syn(syn_unshuffled, column_names):
    ...
    """
    shuff_df= pd.read_csv('.\Data\synthetic_shuffled.csv', sep=';', encoding='latin1')
